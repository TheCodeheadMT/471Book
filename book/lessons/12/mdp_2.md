...
**3. A Space Force reconnaissance satellite receives an immediate reward of $+4$ for capturing an image, and a reward of $+10$ one step later when the image is transmitted to a ground station. If the discount factor is $\gamma = 0.5$, what is the expected two-step return ($G$)?**
